...
class Validation {
	...
	public function checkVendorConfigureOPP( $data, $errors ) {
		...
		foreach ( $vendor_names as $vendor_id => $data ) {
			$errors->add( 'stipe-not-configured', sprintf( /* translators: %1$s: the vendor name, %2$s: the list of products */ esc_html__( '<strong>Error!</strong> You cannot complete your purchase until <strong>%1$s</strong> has enabled OPP as a payment gateway. Please remove %2$s to continue.', 'online-payment-platform-gateway' ), $data['name'], $data['products'] ) );
		}
	}
}
