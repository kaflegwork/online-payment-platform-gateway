...
use OnlinePaymentPlatformGateway\Common\Traits\Singleton;
...
final class Plugin {
	...
	public function data(): array {
		$plugin_data = apply_filters(
			'online_payment_platform_gateway_plugin_data',
			array(
				'settings'               => get_option( 'online-payment-platform-gateway-settings' ),
				'plugin_path'            => untrailingslashit(
					plugin_dir_path( ONLINE_PAYMENT_PLATFORM_GATEWAY_PLUGIN_FILE )  // phpcs:disable ImportDetection.Imports.RequireImports.Symbol -- this constant is global
				),
				'plugin_url'             => untrailingslashit(
					plugin_dir_url( ONLINE_PAYMENT_PLATFORM_GATEWAY_PLUGIN_FILE )  // phpcs:disable ImportDetection.Imports.RequireImports.Symbol -- this constant is global
				),
				'plugin_template_folder' => 'templates',
				'ext_template_folder'    => 'online-payment-platform-gateway-templates',
			/**
			 * Add extra data here
			 */
			)
		...
		return array_merge(
			apply_filters(
				'online_payment_platform_gateway_plugin_meta_data',
				get_file_data(
					ONLINE_PAYMENT_PLATFORM_GATEWAY_PLUGIN_FILE, // phpcs:disable ImportDetection.Imports.RequireImports.Symbol -- this constant is global
					array(
						'name'         => 'Plugin Name',
						'uri'          => 'Plugin URI',
